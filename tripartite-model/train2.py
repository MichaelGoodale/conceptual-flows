    ...
    def validate(model, vision, concepts):
        with torch.no_grad():
            ...
            for concept_idx, name in enumerate(CLASSES):
                plt.scatter(*model.inverse_transform(boundary, concepts[concept_idx])[0].T.cpu().detach(), c=plt.cm.tab20(i), label=name)
                plt.scatter(*model.sample(concepts[concept_idx], n=500)[0].T.cpu().detach(), alpha=0.15, c=plt.cm.tab20(i),)
            plt.legend()
            plt.show()
